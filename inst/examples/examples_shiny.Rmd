...
```{r}
breaks3 <- c(1,2,3,5,6,8,9,10,20,30,40,50)
colors3 <- RColorBrewer::brewer.pal(11, "Spectral")
output$barchart1 <- renderTimeseriesBarChart({
  timeseriesBarChart(sensors$data, sensors$meta, inputId = 'sensor', colors = colors3, breaks = breaks3)
})

```

```{r}
breaks4 <- c(1,2,3,5,6,8,9,10,20,30,40,50)
colors4 <- RColorBrewer::brewer.pal(11, "Spectral")
output$calendar1 <- renderTimeseriesCalendar({
  timeseriesCalendar(sensors$data, sensors$meta, inputId = 'sensor', colors = colors4, breaks = breaks4)
})
```
